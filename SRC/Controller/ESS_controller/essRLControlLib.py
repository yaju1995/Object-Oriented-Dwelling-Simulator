...
logger = CustomLogger(command=True, color='cyan')
...
class essController:
    def __init__(self, rl_agent:Bound_DDPGAgent, mode='Train', resolution: timedelta = timedelta(minutes=1),
        ...
        #                                xlabel='episode',
        #                                ylabel='reward')
        self.live_plotter = LivePlotter4(titles=['Reward','Critic loss','Actor loss', 'None'],
                                         xlabels=['Eps','Eps','Eps','Eps'],
                                         ylabels=['Reward','Critic loss','Actor loss', 'None'])
    def __str__(self):
        return (
            "ESSController(\n"
    ...
    def compute_reward(self, control_time: datetime):  # replace control time wiht self time??
        ...
        normalized_feed_tariff = (feed_tariff_states - tariff_min) / (tariff_max - tariff_min)  # Normalized over max and min
